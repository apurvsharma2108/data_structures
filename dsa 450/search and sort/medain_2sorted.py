...
class Solution:
 ...
 # Now try to find the i where a[i-1] <= b[j] and b[j-1] <= a[i]. So this i is partition around which lies the median.
    def findMedianSortedArrays(self, input1: List[int], input2: List[int]) -> float:
        ...
        while(low<=high):
            ...
            Mx_left_x= -math.inf if(partition_x==0) else input1 [partition_x-1]
            Mn_right_x= math.inf if(partition_x==x) else input1 [partition_x]
            
            Mx_left_y=-math.inf if(partition_y==0) else input2[partition_y-1]
            Mn_right_y=math.inf if(partition_y==y) else input2[partition_y]
